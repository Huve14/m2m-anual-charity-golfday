...
function Companies({ event, version, onRefresh }: { event: EventRecord; version: number; onRefresh: () => void }) {
  ...
  if (selectedCompany) {
    ...
    const allocatedHoles = Array.from(new Set(activeSponsors.flatMap((item) => item.units.flatMap((unit) => { const slot = sponsors?.holeSlots.find((position) => position.id === unit.holeSlotId); return slot ? [slot.holeNumber] : []; })))).sort((a, b) => a - b);
...
function HoleAllocationBoard({ data, companyId, busy, onAllocate, onCreateSlot }: { data: SponsorPayload; companyId: string; busy: boolean; onAllocate: (unitId: string, slotId: string) => Promise<void>; onCreateSlot: (holeId: string, label: string) => Promise<boolean> }) {
  ...
  return <section className="hole-allocation-workspace">
    <div className="hole-course"><div className="fourball-list-heading"><strong>Hole allocation</strong><span>Choose a hole</span></div><div className="hole-number-grid">{numbers.map((number) => {
      ...
      return <button key={number} className={matches ? "company-hole" : ""} aria-pressed={selectedNumber === number} onClick={() => setSelectedNumber(number)}><strong>{String(number).padStart(2, "0")}</strong><span>{occupied.length ? `${occupied.length}/${positions.length} placed` : "Open"}</span>{matches ? <small>Selected company</small> : null}</button>;
    })}</div></div>
    <div className="panel hole-editor"><p className="eyebrow">Sponsor placement</p><h3>Hole {selectedNumber}</h3><p className="muted-copy">Select a confirmed sponsorship for each position. Moving a sponsorship frees its previous position.</p>
      {slots.map((slot) => {
        ...
        const options = units.filter((unit) => unit.commitment.status === "confirmed" && unit.commitment.requiresHole && (!companyId || unit.commitment.eventCompanyId === companyId) && (!slot.sponsorshipTypeId || slot.sponsorshipTypeId === unit.commitment.sponsorshipTypeId));
        return <div className="hole-position" key={slot.id}><strong>{slot.label}</strong>{assigned ? <><h4>{assigned.commitment.companyName}</h4><p>{assigned.commitment.typeName} · unit {assigned.unitNumber}</p><button className="secondary-button" disabled={busy || Boolean(companyId && assigned.commitment.eventCompanyId !== companyId)} onClick={() => onAllocate(assigned.id, "")}>Remove placement</button></> : <label><span>Place sponsorship</span><select aria-label={`Sponsor for hole ${selectedNumber}, ${slot.label}`} value="" disabled={busy || !options.length} onChange={(change) => { if (change.target.value) onAllocate(change.target.value, slot.id); }}><option value="">{options.length ? "Choose company sponsorship" : "No eligible confirmed sponsorships"}</option>{options.map((unit) => <option key={unit.id} value={unit.id}>{unit.commitment.companyName} · {unit.commitment.typeName} · #{unit.unitNumber}{unit.holeSlotId ? ` · move from hole ${data.holeSlots.find((position) => position.id === unit.holeSlotId)?.holeNumber || "?"}` : ""}</option>)}</select></label>}</div>;
      })}
...
function Sponsorships({ event, version, onRefresh, companyId = "" }: { companyId?: string; event: EventRecord; version: number; onRefresh: () => void }) {
  ...
  return <>
    ...
    {data ? <HoleAllocationBoard data={data} companyId={companyFilter} busy={busy} onAllocate={allocate} onCreateSlot={(holeId, label) => action({ action: "createHoleSlot", holeId, label })} /> : <Loading />}
...
function Fourballs({ event, version, onRefresh, companyId = "" }: { companyId?: string; event: EventRecord; version: number; onRefresh: () => void }) {
  ...
  function selectTeam(id: string) {
    ...
    setDirtyForms([]); setSelectedId(id); setPlayerPosition(1); setNotice("");
  }
  ...
  return <>
    ...
    {!loaded.data ? <Loading /> : fourballs.length === 0 ? <Empty title="No client fourballs" copy="Open Packages & new bookings to create your first teams." /> : <>
      <div className="fourball-toolbar">
      ...
      <div className="fourball-workspace">
