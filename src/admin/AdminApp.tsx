...
function Companies({ event, version, onRefresh }: { event: EventRecord; version: number; onRefresh: () => void }) {
  ...
  async function status(company: EventCompany, value: string) { setBusy(true); try { await jsonMutation("/api/v1/admin/companies", "PATCH", { id: company.id, eventId: event.id, relationshipStatus: value }); setLocalVersion((v) => v + 1); } catch (caught) { setError(caught instanceof Error ? caught.message : "Status update failed."); } finally { setBusy(false); } }
  return <><SectionHeader eyebrow="Event relationships" title="Companies" copy="Reuse an existing company or create one directly—no spreadsheet import required." /><ErrorBanner message={error} /><section className="panel"><details className="action-disclosure"><summary>Add company</summary><form className="form-grid compact" onSubmit={add}><label><span>Use existing company</span><select name="companyId" defaultValue=""><option value="">Create a new company</option>{data?.directory.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><label><span>New company name</span><input name="name" placeholder="Leave blank when choosing existing" /></label><label><span>Relationship status</span><select name="relationshipStatus" defaultValue="prospect"><option value="prospect">Prospect</option><option value="pending">Pending</option><option value="confirmed">Confirmed</option></select></label><label><span>Primary contact</span><input name="primaryContactName" /></label><label><span>Contact email</span><input type="email" name="primaryContactEmail" /></label><label><span>Contact phone</span><input name="primaryContactPhone" /></label><FormActions busy={busy} label="Add company" /></form></details></section>{!data ? <Loading /> : data.companies.length === 0 ? <Empty title="No companies in this event" copy="Add a sponsor or fourball company to begin." /> : <div className="data-cards">{data.companies.map((company) => <article key={company.id} className="data-card"><div><Pill value={company.relationshipStatus} /><h3>{company.name}</h3><p>{company.primaryContactName || "No primary contact"}</p><a href={company.primaryContactEmail ? `mailto:${company.primaryContactEmail}` : undefined}>{company.primaryContactEmail || "Email not supplied"}</a></div><label><span>Status</span><select value={company.relationshipStatus} disabled={busy} onChange={(e) => status(company, e.target.value)}><option value="prospect">Prospect</option><option value="pending">Pending</option><option value="confirmed">Confirmed</option><option value="cancelled">Cancelled</option></select></label></article>)}</div>}</>;
}
...
interface SponsorshipCommitment { id: string; eventCompanyId: string; companyName: string; sponsorshipTypeId: string; typeName: string; status: string; quantity: number; confirmedAmountMinor: number; paymentStatus: string; units: Array<{ id: string; unitNumber: number; holeSlotId: string | null }> }
...
function Sponsorships({ event, version, onRefresh }: { event: EventRecord; version: number; onRefresh: () => void }) {
  ...
  return <><SectionHeader eyebrow="Sponsor operations" title="Inventory and hole allocation" copy="Reserved and confirmed sponsorships consume capacity; confirmed units can be allocated to typed hole slots." /><ErrorBanner message={error} />{data ? <div className="inventory-grid">{data.types.map((type) => <article key={type.id} className="inventory-card"><span>{type.category.replaceAll("_", " ")}</span><h3>{type.name}</h3><strong>{usedByType.get(type.id) || 0}<small> / {type.capacity}</small></strong><p>{money(type.priceMinor, event.currency)} · {type.requiresHole ? "Hole allocation" : "No hole required"}</p><div className="capacity-bar"><i style={{ width: `${type.capacity ? Math.min(100, ((usedByType.get(type.id) || 0) / type.capacity) * 100) : 0}%` }} /></div></article>)}</div> : <Loading />}<div className="split-panels"><section className="panel"><details className="action-disclosure"><summary>Configure sponsorship type</summary><form className="stack-form" onSubmit={addType}><label><span>Name</span><input name="name" required /></label><label><span>Category</span><select name="category"><option value="alcoholic_hole">Alcoholic hole</option><option value="non_alcoholic_hole">Non-alcoholic hole</option><option value="branded_hole">Branded hole</option><option value="other">Other</option></select></label><div className="two-fields"><label><span>Capacity</span><input type="number" name="capacity" min="0" required /></label><label><span>Price ({event.currency})</span><input type="number" name="price" min="0" required /></label></div><label className="check-inline"><input type="checkbox" name="requiresHole" />Requires hole allocation</label><FormActions busy={busy} label="Add type" /></form></details></section><section className="panel"><details className="action-disclosure"><summary>Confirm sponsorship</summary><form className="stack-form" onSubmit={addCommitment}><label><span>Company</span><select name="eventCompanyId" required><option value="">Select company</option>{data?.companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label><span>Inventory type</span><select name="sponsorshipTypeId" required><option value="">Select type</option>{data?.types.map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select></label><div className="two-fields"><label><span>Status</span><select name="status" defaultValue="confirmed"><option value="draft">Draft</option><option value="reserved">Reserved</option><option value="confirmed">Confirmed</option></select></label><label><span>Quantity</span><input type="number" name="quantity" min="1" defaultValue="1" required /></label></div><label><span>Confirmed amount ({event.currency})</span><input type="number" name="amount" min="0" required /></label><label><span>Payment</span><select name="paymentStatus"><option value="unpaid">Unpaid</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="waived">Waived</option></select></label><label><span>Invoice reference</span><input name="invoiceReference" /></label><FormActions busy={busy} label="Add sponsorship" /></form></details></section></div><section className="panel"><h3>Confirmed commitments</h3>{data?.commitments.length ? <div className="table-scroll"><table><thead><tr><th>Company</th><th>Type</th><th>Status</th><th>Units</th><th>Amount</th><th>Payment</th></tr></thead><tbody>{data.commitments.map((item) => <tr key={item.id}><td>{item.companyName}</td><td>{item.typeName}</td><td><Pill value={item.status} /></td><td><div className="unit-list">{item.units.map((unit) => <label key={unit.id}><span>#{unit.unitNumber}</span><select value={unit.holeSlotId || ""} disabled={busy || item.status !== "confirmed"} onChange={(e) => allocate(unit.id, e.target.value)}><option value="">Unallocated</option>{data.holeSlots.filter((slot) => !slot.unitId || slot.unitId === unit.id).map((slot) => <option key={slot.id} value={slot.id}>{slot.displayLabel}</option>)}</select></label>)}</div></td><td>{money(item.confirmedAmountMinor, event.currency)}</td><td><Pill value={item.paymentStatus} /></td></tr>)}</tbody></table></div> : <Empty title="No sponsorship commitments" copy="Add a company and confirm its first sponsorship." />}</section></>;
}
