...
const tabs: Array<{ id: Tab; label: string; icon: string }> = [
  { id: "photos", label: "Photos", icon: "13" },
...
function EventTab({ event, tab, version, onRefresh }: { event: EventRecord; tab: Tab; version: number; onRefresh: () => void }) {
  if (tab === "photos") return <AdminPhotos key={event.id} eventId={event.id} eventName={event.name} />;
    ...
    return <div className="company-workspace company-overview">
      ...
      <div className="company-booking-panels">
        ...
        <section className="panel"><header><h3>Sponsorships</h3>{activeSponsors.length ? <button className="text-button" disabled={busy} aria-expanded={editor === "sponsorships"} onClick={() => toggleEditor("sponsorships")}>{editor === "sponsorships" ? "Close details" : "Sponsorship details"}</button> : null}</header>
          ...
          {activeSponsors.length ? activeSponsors.map((item) => <div className="company-booking-row" key={item.id}><div><strong>{item.typeName}</strong>{item.contribution ? <span>Supplying: {item.contribution}</span> : null}<span>{sponsorshipCategoryLabel(sponsors?.types.find((type) => type.id === item.sponsorshipTypeId)?.category)}</span><span>{item.quantity} unit{item.quantity === 1 ? "" : "s"} · {money(item.confirmedAmountMinor, event.currency)}</span><span>{item.units.map((unit) => sponsors?.holeSlots.find((slot) => slot.id === unit.holeSlotId)?.displayLabel || "Location not assigned").join(" · ")}</span>{sponsors ? item.units.map(unit => <SponsorshipTypeChange key={`${unit.id}-${item.sponsorshipTypeId}`} unit={unit} item={item} data={sponsors} busy={busy} onSave={async body => {setBusy(true);setError("");try {await jsonMutation("/api/v1/admin/sponsorships", "POST", {eventId:event.id,...body});setLocalVersion(v=>v+1);onRefresh();setMessage("Hole sponsorship type changed.");return true;} catch(caught){setError(caught instanceof Error ? caught.message : "Could not change hole type.");return false;} finally {setBusy(false);}}} />) : null}</div><div className="company-booking-actions"><Pill value={item.status} /><button className="danger-link" disabled={busy} aria-label={`Remove sponsorship ${item.typeName}`} onClick={() => removeBooking("sponsorships", item.id)}>Remove</button></div></div>) : <p className="muted-copy">No sponsorships booked. Add one above to enter details.</p>}
        </section>
      </div>
    ...
    {data ? <>
      ...
      <section className="panel">
        ...
        {visible.length ? <div className="commitment-list">{visible.map((item) => <article className="commitment-card supplier-record" key={item.id}>
          ...
          {item.status !== "cancelled" ? <details className="action-disclosure"><summary>Location · {item.units.map((unit) => data.holeSlots.find((slot) => slot.id === unit.holeSlotId)?.displayLabel || "Not assigned").join("; ")}</summary><p className="muted-copy">Optional. Choose a hole or a venue position, or leave this contribution without a location.</p>{item.units.map((unit) => <div key={unit.id}><SponsorshipPlacementCard unit={unit} item={item} data={data} busy={busy} onAllocate={allocate} /><SponsorshipTypeChange key={item.sponsorshipTypeId} unit={unit} item={item} data={data} busy={busy} onSave={action} /></div>)}</details> : null}
...
function SponsorshipTypeChange({unit, item, data, busy, onSave}: {unit: SponsorshipCommitment["units"][number]; item: SponsorshipCommitment; data: SponsorPayload; busy: boolean; onSave: (body: Record<string, unknown>) => Promise<boolean>}) {
  const [typeId, setTypeId] = useState(item.sponsorshipTypeId);
  const [editing, setEditing] = useState(false);
  if (!["alcoholic_hole", "non_alcoholic_hole", "branded_hole"].includes(item.category || data.types.find(t => t.id === item.sponsorshipTypeId)?.category || "") || item.status === "cancelled") return null;
  const location = data.holeSlots.find(s => s.id === unit.holeSlotId)?.displayLabel || `Unit ${unit.unitNumber} (unallocated)`;
  return <div className="sponsorship-type-change"><button className="text-button" disabled={busy} aria-expanded={editing} onClick={() => {setTypeId(item.sponsorshipTypeId);setEditing(!editing);}}>Change hole type · {location}</button>{editing ? <form className="stack-form" onSubmit={async e => {e.preventDefault();if(await onSave({action:"changeUnitType",unitId:unit.id,sponsorshipTypeId:typeId}))setEditing(false);}}><label><span>New sponsorship package</span><select value={typeId} disabled={busy} onChange={e => setTypeId(e.target.value)}>{data.types.filter(t => t.id === item.sponsorshipTypeId || t.isActive && ["alcoholic_hole","non_alcoholic_hole","branded_hole"].includes(t.category)).map(t => <option key={t.id} value={t.id}>{t.name} · {sponsorshipCategoryLabel(t.category)}</option>)}</select></label><p className="muted-copy">Changes this unit only. Its location is retained. When splitting a booking, its confirmed amount is divided proportionally; the company total stays the same.</p><button className="secondary-button" disabled={busy || typeId === item.sponsorshipTypeId}>Save hole type</button></form> : null}</div>;
}
