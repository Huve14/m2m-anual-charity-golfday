...
const newGuest = (): Guest => ({ fullName: '', email: '', phone: '', dietaryRequirements: '', attendance: 'pending' });
...
export function GalaDinner({ eventId, exports }: { eventId: string; exports: ReactNode }) {
  ...
  return <>
    ...
    {editing && <form className="panel" onSubmit={(e: FormEvent) => { e.preventDefault(); void save({ action: 'saveParty', id: partyId, name, tableName, guests }); }}>
      <h3>{partyId ? 'Edit dinner party' : 'New dinner party'}</h3><fieldset disabled={busy} className="gala-fieldset"><div className="form-grid compact"><label><span>Party name</span><input required maxLength={160} value={name} onChange={e => setName(e.target.value)} placeholder="John and Jane Smith" /></label><label><span>Table</span><input maxLength={80} value={tableName} onChange={e => setTableName(e.target.value)} placeholder="e.g. Table 4" /></label></div>
      <p>Add each dinner-only guest separately. To include an existing golfer, save this party and use “Dinner details” in the attendee list.</p>
      {guests.map((g, i) => <div className="gala-guest" key={g.id || i}><h4>Guest {i + 1}</h4><div className="form-grid compact"><label><span>Full name</span><input required maxLength={160} value={g.fullName} onChange={e => updateGuest(i, { fullName: e.target.value })} /></label><label><span>Email</span><input type="email" value={g.email} onChange={e => updateGuest(i, { email: e.target.value })} /></label><label><span>Phone</span><input maxLength={40} value={g.phone} onChange={e => updateGuest(i, { phone: e.target.value })} /></label><label htmlFor={`guest-attendance-${i}`}><span>Attendance</span><AttendanceInput id={`guest-attendance-${i}`} value={g.attendance} onChange={attendance => updateGuest(i, { attendance })} /></label><label><span>Dietary requirements</span><textarea maxLength={1000} value={g.dietaryRequirements} onChange={e => updateGuest(i, { dietaryRequirements: e.target.value })} placeholder="e.g. vegetarian, nut allergy; enter None if confirmed" /></label></div><button type="button" className="secondary-button" onClick={() => setGuests(current => current.filter((_, index) => index !== i))}>Remove guest {i + 1}</button></div>)}
    ...
    {player && <form className="panel" onSubmit={e => { e.preventDefault(); void save({ action: 'savePlayer', id: player.id, partyId: player.partyId || null, attendance: player.attendance }); }}><h3>Dinner details · {player.fullName}</h3><fieldset disabled={busy} className="gala-fieldset"><div className="form-grid compact"><label htmlFor="player-attendance"><span>Attendance</span><AttendanceInput id="player-attendance" value={player.attendance} onChange={attendance => setPlayer({ ...player, attendance })} /></label><label><span>Party and shared table</span><select value={player.partyId} onChange={e => setPlayer({ ...player, partyId: e.target.value })}><option value="">No party assigned</option>{data?.parties.map(p => <option value={p.id} key={p.id}>{p.name} · {p.table_name || 'No table'}</option>)}</select></label></div><p>Dietary requirements: {player.dietaryRequirements || 'Not recorded'}. Update the golfer’s details on the Players or Fourballs page; changes appear here automatically.</p><div className="form-actions"><button className="primary-button">{busy ? 'Saving…' : 'Save dinner details'}</button><button className="secondary-button" type="button" onClick={() => setPlayer(null)}>Cancel</button></div></fieldset></form>}
    <section className="panel"><h3>Everyone attending</h3><p>Named players from active fourballs are linked automatically. Empty player slots and cancelled bookings are excluded. Pending guests are not included in confirmed dinner totals.</p><div className="form-grid compact"><label><span>Search attendees, tables or dietary needs</span><input type="search" value={search} onChange={e => setSearch(e.target.value)} /></label><label><span>Attendance filter</span><select value={filter} onChange={e => setFilter(e.target.value)}><option value="all">All attendees</option><option value="confirmed">Confirmed</option><option value="pending">Pending</option><option value="declined">Not attending</option></select></label></div>
    ...
    {!!data?.parties.length && <section className="panel"><h3>Parties and tables</h3><div className="compact-list">{data.parties.map(p => <div key={p.id}><div><strong>{p.name}</strong><span>{p.table_name || 'Table unassigned'} · {attendees.filter(a => a.partyId === p.id && a.attendance === 'confirmed').length} confirmed</span></div><button className="secondary-button" disabled={busy} onClick={() => editParty(p)}>Edit party</button></div>)}</div></section>}
    {exports}
  </>;
}
