...
export function GalaDinner({ eventId, exports }: { eventId: string; exports: ReactNode }) {
  ...
  const visible = attendees.filter(p => (filter === 'all' || (filter === 'active' ? p.attendance !== 'declined' : p.attendance === filter)) && [p.fullName, p.partyName, p.tableName, p.company, p.team, p.dietaryRequirements].join(' ').toLowerCase().includes(search.toLowerCase()));
  const visibleParties = (data?.parties || []).filter(p => filter === 'all' || attendees.some(a => a.partyId === p.id && (filter === 'active' ? a.attendance !== 'declined' : a.attendance === filter)));
  ...
  return <>
    ...
    {!!visibleParties.length && <section className="panel"><h3>Parties and tables</h3><div className="compact-list">{visibleParties.map(p => <div key={p.id}><div><strong>{p.name}</strong><span>{p.table_name || 'Table unassigned'} · {p.guests.length} dinner-only seats · {attendees.filter(a => a.partyId === p.id && a.attendance === 'confirmed').length} confirmed</span></div><button className="secondary-button" disabled={busy} onClick={() => editParty(p)}>Edit party</button>{attendees.some(a => a.partyId === p.id && a.attendance !== 'declined') && <button className="secondary-button" disabled={busy} onClick={() => void save({ action: 'cancelAttendance', partyId: p.id })}>Cancel family attendance</button>}</div>)}</div></section>}
    {exports}
  </>;
}
