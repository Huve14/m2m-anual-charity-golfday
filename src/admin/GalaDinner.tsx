...
export function GalaDinner({ eventId, exports }: { eventId: string; exports: ReactNode }) {
  ...
  return <>
    ...
    <section className="panel"><h3>Guest seating lookup</h3><p>Share this link with guests so they can find their table by name, company or party without signing in. Seating is available while this event is active.</p><a href={`/galadinner?event=${encodeURIComponent(eventId)}`} target="_blank" rel="noreferrer">Open guest seating page</a><p><label>Link to share<input readOnly value={`${typeof window === 'undefined' ? '' : window.location.origin}/galadinner?event=${encodeURIComponent(eventId)}`} onFocus={e => e.target.select()} /></label></p></section>
