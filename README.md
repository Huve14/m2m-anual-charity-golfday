...
Apply `supabase/migrations/20260910174137_gala_dinner_parties.sql` and then `supabase/migrations/20260910201830_gala_attendance_cancellation.sql` before deploying this feature. The two new tables are accessible only through the authenticated admin API; the migration enables RLS and revokes direct client access.

All four player places from non-cancelled fourballs and companies appear automatically with confirmed dinner attendance, even while names are missing. Explicit dinner cancellations override automatic confirmation. Their dietary information is read from the existing player record, so changes by hosts or administrators carry through. Dinner-only parties can be saved with just a name and quantity; each seat starts confirmed and guest details can be added later. The quantity excludes linked golfers, who are counted separately. Blank dietary requirements mean not recorded; enter “None” when confirmed. Party table labels are shared by all members; capacity is not enforced.

**Gala dinner and catering** exports confirmed guests, dietary requirements and table totals. **Complete attendance register** includes those sheets plus every golfer place and dinner-only seat with their pending, confirmed or declined dinner status. Unnamed seats are labelled as awaiting details. Cancelled golf participation is excluded.
