...
export function galaAttendees({ parties = [], settings = [], players = [] }) {
  ...
  const golfers = players.filter(p => p.fourball && p.fourball.booking_status !== 'cancelled' && p.fourball.eventCompany?.relationship_status !== 'cancelled').map(p => {
    ...
    return { id: p.id, source: 'Golfer', fullName: p.full_name?.trim() || `Guest details pending · ${p.fourball.team_name || 'Fourball'} · Player ${p.position || ''}`,  email: p.email || '', phone: p.phone || '', dietaryRequirements: p.dietary_requirements || '', attendance: 'confirmed', partyId: party?.id || '', partyName: party?.name || '', tableName: party?.table_name || '', company: p.fourball.eventCompany?.company?.name || '', team: p.fourball.team_name || '' };
  });
