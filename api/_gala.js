...
export function galaAttendees({ parties = [], settings = [], players = [] }) {
  ...
  const golfers = players.filter(p => p.fourball && p.fourball.booking_status !== 'cancelled' && p.fourball.eventCompany?.relationship_status !== 'cancelled').map(p => {
    ...
    return { id: p.id, source: 'Golfer', fullName: p.full_name?.trim() || `Guest details pending · ${p.fourball.team_name || 'Fourball'} · Player ${p.position || ''}`,  email: p.email || '', phone: p.phone || '', dietaryRequirements: p.dietary_requirements || '', attendance: setting?.attendance === 'declined' ? 'declined' : 'confirmed', partyId: party?.id || '', partyName: party?.name || '', tableName: party?.table_name || '', company: p.fourball.eventCompany?.company?.name || '', team: p.fourball.team_name || '' };
  });
  const guests = parties.flatMap(p => p.guests.map((g, index) => ({ ...g, fullName: g.fullName?.trim() || `Guest details pending · ${p.name} · Guest ${index + 1}`, source: 'Dinner only', partyId: p.id, partyName: p.name, tableName: p.table_name, company: '', team: '' })));
