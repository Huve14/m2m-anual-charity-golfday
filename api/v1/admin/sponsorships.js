...
const actionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("updateCommitment"), eventId: z.string().uuid(), id: z.string().uuid(), status: z.enum(["draft", "reserved", "confirmed", "cancelled"]).optional(), quantity: z.number().int().min(1).max(99).optional(), confirmedAmountMinor: z.number().int().min(0).optional(), invoiceReference: z.string().trim().max(120).nullable().optional(), paymentStatus: z.enum(["unpaid", "partial", "paid", "waived"]).optional(), prizeValueMinor: z.number().int().min(0).max(2147483647).nullable().optional(), contribution: z.string().trim().max(2000).optional(), notes: z.string().trim().max(5000).nullable().optional() }),
  z.object({ action: z.literal("createSupplier"), eventId: z.string().uuid(), eventCompanyId: z.string().uuid(), contribution: z.string().trim().min(1).max(2000), prizeValueMinor: z.number().int().min(0).max(2147483647).nullable().optional(), holeSlotId: z.string().uuid().nullable().optional() }),
...
async function list(req, res) {
  ...
  sendJson(res, 200, {
    ...
    commitments: commitments.data.filter((item) => req.query?.includeCancelled === "true" || (item.status !== "cancelled" && item.eventCompany?.relationship_status !== "cancelled")).map((item) => ({
      ...
      paymentStatus: item.payment_status, contribution: item.contribution || "", prizeValueMinor: item.prize_value_minor ?? null, notes: item.notes || "",
...
async function mutate(req, res) {
  ...
  if (input.action === "createSupplier") {
    ...
    if (input.prizeValueMinor !== undefined) changes.prize_value_minor = input.prizeValueMinor;
