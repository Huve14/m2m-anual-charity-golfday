...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === "players") {
      ...
      const { data, error } = await client.from("m2m_sponsorship_commitments").select("status,quantity,confirmed_amount_minor,payment_status,invoice_reference,contribution,prize_value_minor,type:m2m_sponsorship_types(name),eventCompany:m2m_event_companies(company:m2m_companies(name)),units:m2m_sponsorship_units(unit_number,slot:m2m_hole_sponsorship_slots(label,location_name,hole:m2m_event_holes(label)))").eq("event_id", eventId);
      if (error) throw fromSupabase(error);
      rows = [["Company", "Sponsorship", "Status", "Unit", "Location", "Contribution", "Prize value (ZAR, contribution total)", "Amount", "Invoice", "Payment"], ...data.flatMap((item) => (item.units || []).map((unit) => [item.eventCompany?.company?.name, item.type?.name, item.status, unit.unit_number, unit.slot ? `${unit.slot.hole?.label || unit.slot.location_name} · ${unit.slot.label}` : "Unallocated", item.contribution, item.prize_value_minor == null ? "" : (item.prize_value_minor / 100).toFixed(2), item.confirmed_amount_minor, item.invoice_reference, item.payment_status]))];
    } else {
